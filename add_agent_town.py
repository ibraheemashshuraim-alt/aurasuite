...
def enhance_file(filepath):
    ...
    # 2. Find the injection point after ADMIN QUICK VIEW
    # Look for:
    #                   </div>
    #                 )}
    #   
...
"""
    ...
    # We want to insert this block directly after the ADMIN QUICK VIEW block.
    # Pattern to match the end of ADMIN QUICK VIEW block:
    # 
    #                   </div>
    #                 )}
    # 
    ...
    pattern = r'(                 \</div>\s*\n\s*\)\}\s*\n)(\s*\{\/\*\s*CLIENT VIEW IN DASHBOARD\s*\*\/})'
    
    if re.search(pattern, code):
        code = re.sub(pattern, r'\1' + agent_town_ui + r'\2', code, count=1)
