...
pattern_effect = r"useEffect\(\(\) => \{ const saved = localStorage.getItem\('aura_worker_tab'\); if \(saved\) setActiveTab\(saved\); \}, \[\]\);"
replacement_effect = """useEffect(() => { const saved = localStorage.getItem('aura_worker_tab'); if (saved) setActiveTab(saved); }, []);
  useEffect(() => {
...
print("Updated login/page.js successfully")
