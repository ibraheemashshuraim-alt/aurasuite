export async function POST(request) {
  try {
    ...
    const { to, name, cardNumber, username, tempPassword, orgName } = body;
...
<body style="margin:0;padding:0;background:#0a0514;font-family:'Segoe UI',Arial,sans-serif;">
  <div style="max-width:520px;margin:40px auto;background:linear-gradient(135deg,#1a0f2e,#0d0820);border:1px solid rgba(139,92,246,0.3);border-radius:20px;overflow:hidden;">
    <!-- Header -->
    ...
    <div style="padding:32px;">
      <p style="color:#c4b5fd;font-size:14px;margin:0 0 24px;">Hello <strong style="color:#fff;">${name}</strong>, your access to <strong style="color:#a78bfa;">${orgName}</strong> is ready.</p>
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [to],
        subject: `Your AuraSuite Access Card - ${cardNumber}`,
        html
      })
    });
